...
import { setOpenUpdateDialoge } from "@/lib/store/slicer";
...
export default function UpdateService() : React.ReactNode {
  ...
  const openUpdateDialoge = useSelector((state:RootState)=> state.admin.openUpdateDialoge)
  ...
  return <Dialog open={openUpdateDialoge}>
    <DialogContent className="sm:max-w-md !rounded-none [&>button]:hidden">
      <DialogHeader className="!rounded-none">
        <DialogTitle className={`${isRTL ? "text-start" : ""}`}>Modifier le service</DialogTitle>
      ...
      <DialogFooter className="sm:justify-start">
        <DialogClose asChild>
          <Button onClick={(e)=>dispatch(setOpenUpdateDialoge(false))} type="button" variant="secondary" className="cursor-pointer bg-[#061f46] text-white rounded-none hover:bg-[#061f46]/80">
            Fermer
          </Button>
        </DialogClose>
      </DialogFooter>
