    ...
    <Table>
      <TableHeader className="bg-[#061f46]">
        <TableRow className="*:p-3 *:pl-2 *:text-base *:text-white">
          <TableHead className={isRTL ? "text-start" : ""}>Nom de services</TableHead>
          <TableHead className={isRTL ? "text-start" : ""}>Description</TableHead>
      ...
      <TableBody className="*:p-3 *:h-12 *:dark:text-white">
        {
          [
            ...
            return <TableRow key={idx} className={` ${(idx % 2 == 0) ? 'bg-[#061f46]/5' : ''}`}>
              {
                Object.values(d).map((value,id)=>{
                  return <TableCell key={id} className={`${ (id==0) && "font-medium"}`}>{String(value) ?? ''}</TableCell>
                })
              }
              {
                <TableCell className="flex items-center gap-3">
                  ...
                  <div className="p-1.5 bg-black/10 dark:bg-white/10 cursor-pointer hover:bg-black/15 dark:hover:bg-white/15">
                    <Trash onClick={()=>dispatch(setOpenDeleteDialoge(true))} className="text-[#061f46]/80 dark:text-white" size={23}/>
                  </div>
                </TableCell>
              }
            </TableRow>
